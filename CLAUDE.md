...
셋 중 하나라도 누락되면 `/yeoboya-setup`이 차단한다. 추가로 setup은 현재 repo의 하네스 **부트스트랩**을 확인해 `workspace.json.harness.bootstrapped`에 기록한다. 부트스트랩 판정은 **루트 문서 존재**(`CLAUDE.md` + `docs/CONVENTIONS.md` + 내용 있는 `docs/rules/TESTING.md`)로만 한다 — 이 문서는 `harness-root` 1회 실행으로 만들어진다. `harness-module`(leaf 모듈별 CLAUDE.md)은 모듈이 있는 repo에서만 유용하며 부트스트랩 게이트의 선행조건이 아니다.

단, **플래그를 쓰는 주체는 오직 setup이다**. `harness-root`는 문서만 생산하고 `bootstrapped`를 건드리지 않으며, write-code 등 다른 스킬은 이 플래그를 **읽기만** 하고 repo를 재스캔하지 않는다(state-schema §3). 따라서 harness-root 실행 후에는 반드시 `/yeoboya-setup`을 **다시 호출**해 문서 존재를 재검증해야 `bootstrapped=true`로 확정된다.
...
`links`의 권위 출처는 Notion(과제 row의 자식 페이지)이고 task.json.links는 캐시다. 문서 세부작업 스킬은 진입 시 `yeoboya-publish-notion mode="sync-links"`로 links를 동기화하며, 매칭·쓰기는 `hooks/lib/sync-links.js`(결정적 node)가 수행한다.
...
- **user-invocable 진입은 5개**: `/yeoboya-setup`, `/yeoboya-create-task`, `/yeoboya-choose-subtask`, `/yeoboya-insights`(마찰 로그 분석), `/yeoboya-edit-task`(변경 전파). 뒤 둘은 세부작업이 아닌 choose-subtask 흐름 밖의 독립 도구다.
- 세부작업 스킬은 모두 `user-invocable: false`. `choose-subtask`이 Skill 도구로 trigger한다
- **세부작업 단위 세션 분리 권장**: 세부작업 완료 후 새 세션에서 `/yeoboya-choose-subtask` 재호출
