...
**QA 시나리오(write-qa)만 예외**: 마크다운 자식 페이지가 아니라 작업 row **자식 데이터베이스**로 생성된다(제목 "QA 시나리오"). 행=테스트 케이스, 컬럼=`케이스 ID`(title)·`액터`·`카테고리`·`시나리오`·`관련 API / Socket`·`테스트 결과`(select 옵션은 write-qa SKILL §3). 행 본문에 `사전 조건`/`테스트 단계`/`기대 결과`. `테스트 결과`는 신규 행 `미테스트`이고 QA 수행 시 사람이 채운다.
...
- 재게시는 **append-only**: 기존 케이스 ID는 보존(사람이 입력한 테스트 결과 보호), 새 케이스만 추가. row 본문 제목2 삽입 로직은 DB 산출물에 적용되지 않는다.
