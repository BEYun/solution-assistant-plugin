...
작업목록 산출물(정책서, UI 흐름도, 데이터 흐름도, 통신 명세서, QA 시나리오, 기획서 검토)은 해당 작업 DB **row의 자식(child) 페이지**로 생성된다. 제목은 항상 플러그인 경유 정규값(`hooks/lib/constants.json`의 `KEY_TO_TITLE` 값)이다. `sync-links`는 row의 자식 페이지를 나열해 제목 정확 일치로 `work.json.links`를 채운다.
