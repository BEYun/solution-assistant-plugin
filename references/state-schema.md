...
`notion.workDbDataSourceUrl`/`workerPageId`: setup-workspace가 §2.5에서 Notion 검색으로 확정해 캐시한 값(사용자 자유입력 아님). `workDbDataSourceUrl`은 선택 서비스의 작업 DB data source, `workerPageId`는 작업자 이름으로 매칭한 멤버 페이지. `domainMapping` 필드는 제거됨 — 작업의 도메인은 Notion 작업 row의 `도메인` select가 권위 출처이며 workspace는 보유하지 않는다.
