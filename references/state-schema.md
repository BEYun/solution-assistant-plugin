...
- `taskType` ∈ {`feature`, `update`, `bugfix`} — create-work 라벨 + **choose-subtask 세부작업 뷰 분기 키**(`SUBTASK_GROUPS`/`SUBTASK_LABELS` §4). 그 외 분기에는 쓰지 않는다.
...
write-code가 산출하고 하네스 `work`이 입력으로 받는 **코드 과제 계획서**. write-code가 소유하며, work은 이를 읽어 plan-reviewer(7축)·TDD·완료기준 검증을 수행한다. (이전의 phase 기반 진행 추적 방식을 대체 — phase 개념은 폐기됐고 진행 상태는 work이 `.harness/runs/run-{id}.md`로 관리한다.)

고정 섹션:

```markdown
## 요구사항
<task.json.name + 선행 Notion 산출물(정책서/도메인/데이터 흐름도) 요약>
...
**선택 섹션 (링크 제공 시에만)**: write-code(§3.1a)가 디자인/Swagger 링크를 받아 컨텍스트를 주입할 때 아래 두 섹션을 **고정 섹션 뒤에** 추가한다. 링크가 없으면 생략한다. 진행/완료 상태를 담지 않으며 `work`이 참고 자료로 읽는다. 링크를 받으면 대응 완료기준을 파생해 `## 완료기준`에도 자동 추가한다(사용자 확정).

```markdown
## 디자인 참조        (디자인 링크 제공 시에만)
- 소스: Figma <링크>
- 화면/컴포넌트: <ID·이름 목록>
- 디자인 토큰: 색상/타이포/간격 <요약>

## API 참조           (Swagger URL 제공 시에만)
- 소스: Swagger <URL>  (사내망, curl 취득)
- 엔드포인트: `METHOD /path` — 요약
- 요청/응답 DTO: <스키마 요약>
```

plan.md 존재 = write-code가 이미 계획을 세우고 work에 위임을 시작했음(첫 호출 vs 재개의 분기 신호). 진행/완료 상태는 plan.md에 두지 않는다.

**변경 전파 시 델타 재작성**: `edit-work`가 코드 재작성을 트리거할 때, write-code에 넘기기 전 이 plan.md를 **수정 델타로 재프레이밍**한다 — `## 요구사항`을 "기존 `<파일>` 동작에서 다음만 변경… 기존 코드 유지, 전체 재구현 금지"로, `## 참고 코드`에 기존 구현 경로를, `## 완료기준`에 변경 동작 + 회귀 스위트를 둔다. write-code는 plan.md가 존재하므로 재개 분기로 이 델타 plan을 work에 전달한다. "기존 코드를 없애고 새로 짜는" 위험을 막는 3중 방어(델타 지시·참고 코드 앵커·회귀 완료기준)의 근거 문서다. 상세는 `docs/superpowers/specs/2026-07-02-edit-work-change-propagation-design.md` §4.1.

## 3. `.assistant/workspace.json`
...
`activeTask`: most recently started or resumed work. Updated by `create-work` and `choose-subtask`. Used by `session-resume` hook.
`notion.workDbDataSourceUrl`/`workerPageId`: setup이 §2.5에서 Notion fetch로 확정해 캐시한 값(사용자 자유입력 아님). `workDbDataSourceUrl`은 `서비스 목록 → <서비스> → 과제 목록 → 과제 DB` 계층을 타고 내려가 얻은 선택 서비스의 과제 DB data source. `workerPageId`는 워크스페이스 계정(get-users)이 아니라 공용 `팀원 목록` DB에서 `이름`으로 매칭한 **행 페이지**(notion-schema.md §1 참조). `domainMapping` 필드는 제거됨 — 과제의 도메인은 Notion 과제 row의 `도메인` select가 권위 출처이며 workspace는 보유하지 않는다.
`harness.bootstrapped`: setup이 현재 repo의 하네스 문서(CLAUDE.md + docs/CONVENTIONS.md + docs/rules/TESTING.md, 그리고 TESTING.md 테스트 명령 비어있지 않음) 존재를 **1회 확인**한 결과. `true`여야 write-code가 `work`을 호출한다. 다른 스킬은 이 플래그만 읽고 repo를 재스캔하지 않는다. `checkedAt`은 `TZ=Asia/Seoul date +%Y-%m-%d`(감사용, 선택).

`design.tool` ∈ {`figma`, `zeplin`, `null`} — setup이 §2에서 서비스가 쓰는 디자인 툴을 물어 기록한다(서비스마다 다름, 추론 금지). `connected`는 노출 도구에 그 툴의 대표 read 도구(`constants.json DESIGN_TOOLS[*].detectToolSuffixes`) 존재 여부로 판정하는 **best-effort** 결과 — 미연결이어도 기록만 하고 차단하지 않는다. **하드 선행조건이 아니다**(Notion·하네스와 달리). write-code가 첫 호출 시 `design.tool`을 읽어 디자인 링크 물음 여부·대상 툴을 정한다(§3.1a). 구버전 workspace.json에 `design` 키가 없으면 `tool=null`로 취급(하위 호환). `checkedAt`은 `TZ=Asia/Seoul date +%Y-%m-%d`.
...
# 병렬 배열. 산출물 자식 페이지를 과제 DB row에 publish할 때, row 본문에서 그 자식 페이지
...
# write-code는 phase 상수를 두지 않는다. 코드 과제는 하네스 `work` 닫힌 루프에 위임된다.
...
`WORKTYPE_LABEL`은 `create-work`에서 과제 유형 select 값 결정에 쓰인다.
...
- `links` 동기화는 `hooks/lib/sync-links.js`가 담당: 과제 row 자식 페이지 `[{title,id}]`를 stdin으로 받아 `resolveKey`(constants.json)로 매칭·병합·atomicWrite. LLM은 links를 직접 쓰지 않는다.
...
3. **provenance 기록** (seed의 실제 기원을 메타/헤더 + 변경 이력 양쪽에):
   ...
   |---|---|---|
   | referenceTask 노션 문서 | `<referenceTask> (노션 링크)` | `<referenceTask>` |
   | 자기 재publish + referenceTask 설정 | `<referenceTask> (노션 링크)` | `<referenceTask>` |
   | 자기 재publish + referenceTask 없음 | `—` | `—` |
   | 코드베이스 (분기 B) | `코드베이스 (<지정 경로>)` | `코드베이스: <경로>` |

   분기 B인데 `referenceTask`도 설정된 경우(referenceTask에 해당 문서가 노션에 없음) 메타에 보조로 `(참고 과제: <referenceTask>)`를 병기할 수 있다.
