---
name: solution-draw-ui-flow
description: "solution-choose-subtask이 이 세부작업을 trigger할 때만 사용한다. 직접 호출 금지. 고유 ID를 가진 화면, 화면별 고유 ID를 가진 사용자 액션, 그리고 화면 전환 다이어그램을 정의한다. 모든 화면-액션 쌍이 ID를 갖는지 자체 검증한다(draw-data-flow가 ID 기반 연결에 사용). 'UI 흐름도' 제목의 Notion 페이지를 게시한다."
user-invocable: false
---

# solution-draw-ui-flow
