...
description: 사용자가 /solution-insights를 호출하거나 '마찰 분석', '불편 로그 분석', '개선 인사이트', '과제 불편 대시보드'를 원할 때 사용한다. `.assistant/improvement-log.jsonl`을 집계해 단일 HTML 대시보드(`.assistant/insights.html`)를 만들고 원인 분석 서술을 채운 뒤 브라우저로 열도록 안내한다. 세부작업이 아니며 choose-subtask 흐름 밖의 독립 분석 도구다.
user-invocable: true
---

# solution-insights — 과제 불편 분석
...
4. **HTML 생성**: `node ${CLAUDE_PLUGIN_ROOT}/hooks/insights.js html .assistant/.insights-narrative.html` 를 실행한다.
5. **안내**: 출력된 경로(`.assistant/insights.html`)를 브라우저로 열라고 안내한다. (gitignore되어 로컬 전용.)
