---
name: solution-write-code
description: "solution-choose-subtask이 이 세부작업을 trigger할 때만 사용한다. 직접 호출 금지. 선행 Notion 산출물(정책서/도메인/데이터 흐름도)과 하네스 문서를 읽어 .assistant/<과제번호>/plan.md(코드 과제 계획서)를 작성하고, task.json.codeBaseSha를 기록한 뒤, 하네스 플러그인의 work 닫힌 루프에 구현을 위임한다. phase 시스템은 더 이상 쓰지 않는다. solution-publish-notion을 호출하지 않는다."
...
- **하네스 부트스트랩 가정**: `.assistant/workspace.json`의 `harness.bootstrapped`를 읽는다(repo 스캔 아님 — 플래그 1회 읽기). `true`가 아니면 아래 안내 후 **즉시 종료**(work을 호출하지 않는다):
  ...
  /harness-root 실행(leaf 모듈이 있으면 이어서 /harness-module) 후 /solution-setup을 다시 호출해 부트스트랩을 확정한 뒤 진행하세요.
  ```
...
Skill 도구로 하네스 `work` 스킬을 호출하고, plan.md를 입력으로 넘긴다(이름 충돌 시 `solution-workflow-v2:work`):
...
work이 도중 중단(harness-check 사람 게이트, bug-fix 한도 등)되면 work의 보고를 그대로 사용자에게 전달하고(`codeWriteDone` 미기록), 재개는 `/solution-choose-subtask → write-code`(§4)로 안내한다.
