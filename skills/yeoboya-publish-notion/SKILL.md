...
- `mode`: "dispatch" | "dispatch-db" | "sync" | "sync-links" | "list-children"
  ...
  - `dispatch-db`: 세부 작업 산출물을 **마크다운 페이지가 아니라 작업 row 자식 데이터베이스(테이블)**로 게시. payload: `key`, `schema`(CREATE TABLE DDL), `rows`. 현재 사용처는 write-qa(QA 시나리오 테이블) 하나(§4.5).
...
산출물이 마크다운 페이지가 아니라 **작업 row 자식 데이터베이스(테이블)**인 경우(write-qa). DB 제목은 `KEY_TO_TITLE[key][0]`(예 "QA 시나리오") — 이래야 `sync-links`의 제목 매칭(`TITLE_TO_KEY`)이 같은 키로 잡는다.

1. `work`로 work.json 로드 → `links[key]` 확인 (있으면 기존 DB 페이지 id).
2. workspace.json 로드 → `notion.workDbDataSourceUrl`. notion-writer `query`로 작업 row pageId 조회.
3. **신규(`links[key]` 없음)**:
   - notion-writer `create-database`(pageId=작업 row, title=DB 제목, schema=DDL) → `{ dbId, dataSourceId }`.
   - notion-writer `create-rows`(dataSourceId, rows) → 행 일괄 생성.
   - **링크 기록**: 생성 직후 §5.5 `sync-links`와 **동일 절차**를 실행한다 — notion-writer `list-children`(작업 row)로 자식(이제 새 DB 포함) `[{title,id}]`를 나열해 `node ${CLAUDE_PLUGIN_ROOT}/hooks/lib/sync-links.js <work>`에 파이프. 헬퍼가 제목 매칭으로 `links[key]=dbId`를 결정적으로 기록한다(별도 기록 코드 없음 — 기존 테스트된 경로 재사용).
4. **재게시(`links[key]` 있음)** — 사람이 입력한 `테스트 결과`/편집을 보존하기 위해 **append-only**:
...
yeoboya-publish-notion 호출 파라미터:
  ...
  (dispatch만)
    ...
    properties?:
      ...
      Android_완료?: boolean
  (dispatch-db만)
    key: "<세부 작업 키>"                   # DB 제목은 KEY_TO_TITLE[key][0]
