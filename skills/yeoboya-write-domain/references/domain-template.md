...
> 용어 / 파라미터 / 역할 / 정책 카탈로그 / 예외·롤백은 정책서 직접 참조.
> 본 문서는 엔지니어링 elaborate 영역 (엔터티 · 역할별 시나리오 · 상태 머신 · 결정 필요 항목)만 다룬다.
...
도메인에 등장하는 역할과 각 역할이 트리거하거나 관여하는 주요 시나리오. 어휘는 §3 전이 규칙 `역할` 열과 일치한다.
